    ...
    >
      {logs.length === 0 ? (
        <EmptyPanel
          ...
          description="เริ่มบันทึกงานแรกของวันนี้ได้ที่ Work Logs"
        />
      ) : (
        <div className="space-y-3">
          {logs.map((log) => {
            ...
            return (
              <div
                key={log.id}
                className="rounded-xl border border-border bg-background/60 p-3 transition-colors hover:bg-background"
              >
